...
def findPixelsofInterestPerSensor(num_anchorPoints_perSensor,labeledMatrixPixel,unwantedPixelsCorners1,intersection_sizes,coordSensors,sensorRadius,N):
    ...
    for sensor in range(N):
        ...
        for num_samples in range(num_anchorPoints_perSensor):
            coordCurrSensor = coordSensors[sensor]
            check = 0 # check if the new sampled point falls within the radius of interest of the sensor
            while check == 0 :
                # random angle
                alpha = 2 * math.pi * random.random()
                # random radius
                random_radius = sensorRadius * math.sqrt(random.random())
                # calculating coordinates
                temp_x_sampledPoint = random_radius * math.cos(alpha) + coordCurrSensor[0]
                temp_y_sampledPoint = random_radius * math.sin(alpha) + coordCurrSensor[1]
            ...
            index_pixel_ofInterest_perSensor = np.array([temp_x_sampledPoint,temp_y_sampledPoint])
            label_index_pixel_ofInterest_perSensor = labeledMatrixPixel[int(np.shape(labeledMatrixPixel)[0]-index_pixel_ofInterest_perSensor[1]-1)][int(index_pixel_ofInterest_perSensor[0])]
            
            obstructedPixelsPerSensors[sensor].append([label_index_pixel_ofInterest_perSensor])
            index_obstructedPixelsPerSensors[sensor].append([index_pixel_ofInterest_perSensor])
        
        
    return obstructedPixelsPerSensors,index_obstructedPixelsPerSensors


def placeWeightPerSensor(num_anchorPoints_perSensor,labeledMatrixPixel,unwantedPixelsCorners1,intersection_sizes,N):
    weightedRegionsPerSensor = []

    for sensor in range(N):
        weightedRegionsPerSensor.append([])
        weightedRegionsPerSensor[sensor] = 1
...
def weightPixels(obstructedPixelsinBox,index_obstructedPixelsPerSensorsinBox,weightedRegionsPerSensor,labeledMatrixPixel,unwantedPixelsCorners1,intersection_sizes,N):
    ...
    for sensor in range(N):
        ...
        for num_samples in range(len(index_obstructedPixelsPerCurrentSensor)):
            curr_index_obstructedPixelsPerCurrentSensor = index_obstructedPixelsPerCurrentSensor[num_samples][0]
            # Weight the corresponding pixels
            weightedMap[int(np.shape(labeledMatrixPixel)[0]-1-curr_index_obstructedPixelsPerCurrentSensor[1])][int(curr_index_obstructedPixelsPerCurrentSensor[0])] += weightPerCurrentSensorRegion
...
def main(T=int(1e2)): 
    ...
    numIter = 1
    prob_road2 = 0.3
    num_anchorPoints_perSensor = np.arange(1,52,5)
...
#             coordSensors = np.concatenate((xcoordSensors,ycoordSensors),axis=1)             
             ...
             for kk in tqdm(range(len(num_anchorPoints_perSensor))):  
                 
                 startTime = time.time()
                 # Step 1: Find pixels of interest per sensor
                 obstructedPixelsinBox_1, index_obstructedPixelsPerSensorsinBox_1 = findPixelsofInterestPerSensor(num_anchorPoints_perSensor[kk],labeledMatrixPixel,unwantedPixelsCorners1,intersection_sizes, coordSensors, sensorRadius, N[ii])
                 # Step 2: Weight pixels per sensor
                 weightedRegionsPerSensor_1 = placeWeightPerSensor(num_anchorPoints_perSensor[kk],labeledMatrixPixel,unwantedPixelsCorners1,intersection_sizes,N[ii])
    ...
    # =============================================================================
                 ...
                 for mm in range(num):
                    ...
                    weightedAgeTypicalSensor_1 = computeWeightedAgeofTypicalSensor_1(tempselectedSensorsSensSelec_1, sensorRadius, coordSensors, coordPixels, labeledPixels, labeledMatrixPixel, obstructedPixelsinBox_1[mm][0], weightedMap_1, weightedRegionsPerSensor_1[mm], lam, d, N[ii], capacity, mu, pixelLength, pixelWidth, k)
                    ...
                    weightedAgeTypicalSensor_2 = computeWeightedAgeofTypicalSensor_1(tempselectedSensorsSensSelec_2, sensorRadius, coordSensors, coordPixels, labeledPixels, labeledMatrixPixel, obstructedPixelsinBox_1[mm][0], weightedMap_1, weightedRegionsPerSensor_1[mm], lam, d, N[ii], capacity, mu, pixelLength, pixelWidth, k)
