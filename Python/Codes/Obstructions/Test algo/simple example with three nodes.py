...
def findPixelsofInterestPerSensor(labeledMatrixPixel,boxDim,N):
    ...
    # Anchor point between 2 overlapping coverages of the 2 sensors
    index_pixel_overlap = np.array([boxDim[0]/2,boxDim[1]/2])
    label_pixel_overlap = labeledMatrixPixel[int(np.shape(labeledMatrixPixel)[0]-index_pixel_overlap[1]-1)][int(index_pixel_overlap[0])]
    
    # Anchor point in non-overlapping region
    index_pixel_no_overlap = np.array([boxDim[0] - 50, boxDim[1]/2])
    label_pixel_no_overlap = labeledMatrixPixel[int(np.shape(labeledMatrixPixel)[0]-index_pixel_no_overlap[1]-1)][int(index_pixel_no_overlap[0])]
    ...
    for sensor in range(N):
        ...
        obstructedPixelsPerSensors[sensor].append([label_pixel_overlap,label_pixel_no_overlap])
        index_obstructedPixelsPerSensors[sensor].append([index_pixel_overlap,index_pixel_no_overlap])
...
def weightPixels(obstructedPixelsinBox,index_obstructedPixelsPerSensorsinBox,weightedRegionsPerSensor,labeledMatrixPixel,boxDim,N):
    ...
    for sensor in range(1):
        ...
        index_pixel_overlap = index_obstructedPixelsPerCurrentSensor[0][0]
        index_pixel_no_overlap = index_obstructedPixelsPerCurrentSensor[0][1]
        
        weightPerCurrentSensorRegion = weightedRegionsPerSensor[sensor]
        # Weight the corresponding pixels
        weightedMap[int(np.shape(labeledMatrixPixel)[0]-1-index_pixel_overlap[1])][int(index_pixel_overlap[0])] += 1
        weightedMap[int(np.shape(labeledMatrixPixel)[0]-1-index_pixel_no_overlap[1])][int(index_pixel_no_overlap[0])] += 1
...
def main(T=int(1e2)): 
    ...
    for ii in tqdm(range(len(N))):
         ...
         for jj in range(numIter):
             ...
             coordSensors.append(np.array([boxDim[0]/2 - 10, boxDim[1]/2]))
             coordSensors.append(np.array([boxDim[0]/2 + 10, boxDim[1]/2]))
             coordSensors.append(np.array([boxDim[0]-sensorRadius, boxDim[1]/2]))                   
...
#             coordSensors = np.concatenate((xcoordSensors,ycoordSensors),axis=1)             
             ...
             weightedMap_1 = weightPixels(obstructedPixelsinBox_1,index_obstructedPixelsPerSensorsinBox_1,weightedRegionsPerSensor_1,labeledMatrixPixel,boxDim,N[ii])
