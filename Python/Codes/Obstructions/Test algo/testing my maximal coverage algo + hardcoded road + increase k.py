...
def findPixelsofInterestPerSensor(labeledMatrixPixel,rectangleLength,rectangleWidth,N,num_anchor_points):
    obstructedPixelsPerSensors = []
    index_obstructedPixelsPerSensors = []
    coordAnchorPoints = []
    label_anchor_point = []
    inter_anchor_point_dist = 50
    # Pixels of interest in the different regions:
    # Road 1: Left side
#    index_pixel_road1_left = np.array([unwantedPixelsCorners1[1][0],unwantedPixelsCorners1[2][1]+intersection_sizes[1]/2])
#    label_pixel_road1_left = labeledMatrixPixel[int(np.shape(labeledMatrixPixel)[0]-index_pixel_road1_left[1]-1)][int(index_pixel_road1_left[0])]
#    
#    # Road 1: Right side
#    index_pixel_road1_right = np.array([unwantedPixelsCorners1[1][0]+intersection_sizes[0],unwantedPixelsCorners1[2][1]+intersection_sizes[1]/2])
#    label_pixel_road1_right = labeledMatrixPixel[int(np.shape(labeledMatrixPixel)[0]-index_pixel_road1_right[1]-1)][int(index_pixel_road1_right[0])]
#    
#    # Road 2: Up side
#    index_pixel_road2_up = np.array([unwantedPixelsCorners1[1][0]+intersection_sizes[0]/2,unwantedPixelsCorners1[2][1]+intersection_sizes[1]])
#    label_pixel_road2_up = labeledMatrixPixel[int(np.shape(labeledMatrixPixel)[0]-index_pixel_road2_up[1]-1)][int(index_pixel_road2_up[0])]
#    
#    # Road 2: Down side
#    index_pixel_road2_down = np.array([unwantedPixelsCorners1[1][0]+intersection_sizes[0]/2,unwantedPixelsCorners1[2][1]])
#    label_pixel_road2_down = labeledMatrixPixel[int(np.shape(labeledMatrixPixel)[0]-index_pixel_road2_down[1]-1)][int(index_pixel_road2_down[0])]
    nn = 0
    while nn < num_anchor_points:
         check = 0
         ...
         check1 = 0
         for mm in range(len(coordAnchorPoints)):
            if temp_newSensor[0][0] == coordAnchorPoints[mm][0] and temp_newSensor[0][1] == coordAnchorPoints[mm][1]:
                check1 = 1
                break
                # Check that the dist between the new sensor and other sensors is at least: sqrt(L^2+W^2)
         if check1 == 0: 
            if not list(coordAnchorPoints):
                coordAnchorPoints = np.concatenate((temp_x,temp_y),axis=1)
                nn += 1
                coordAnchorPoints = list(coordAnchorPoints)
                label_anchor_point.append(labeledMatrixPixel[int(np.shape(labeledMatrixPixel)[0]-coordAnchorPoints[0][1]-1)][int(coordAnchorPoints[0][0])])
            else:
                for mm in range(len(coordAnchorPoints)):
                    if temp_newSensor[0][1] == coordAnchorPoints[mm][1]:
                        if np.linalg.norm(temp_newSensor[0][0] - coordAnchorPoints[mm][0]) < inter_anchor_point_dist: #np.sqrt((carLengthScaled)**2+(carWidthScaled)**2):
                            check = 1
                            break                        
                if check == 0:
                    coordAnchorPoints.append(np.concatenate((temp_x,temp_y),axis=1)[0])   
                    label_anchor_point.append(labeledMatrixPixel[int(np.shape(labeledMatrixPixel)[0]-temp_y-1)][int(temp_x)])
                    nn += 1
    ...
    for sensor in range(N):
        ...
        obstructedPixelsPerSensors[sensor].append(label_anchor_point)
        index_obstructedPixelsPerSensors[sensor].append(coordAnchorPoints)
        
        
    return obstructedPixelsPerSensors,index_obstructedPixelsPerSensors


def placeWeightPerSensor(labeledMatrixPixel,unwantedPixelsCorners1,intersection_sizes,N,num_anchor_points):
    weightedRegionsPerSensor = []

    for sensor in range(N):
        weightedRegionsPerSensor.append([])
        weightedRegionsPerSensor[sensor] = 1/num_anchor_points
        
    return weightedRegionsPerSensor


def weightPixels(obstructedPixelsinBox,index_obstructedPixelsPerSensorsinBox,weightedRegionsPerSensor,labeledMatrixPixel,unwantedPixelsCorners1,intersection_sizes,N):
    weightedMap = np.zeros(np.shape(labeledMatrixPixel))
    
    for sensor in range(N):
        weightPerCurrentSensorRegion = weightedRegionsPerSensor[sensor]
        for ii in range(len(index_obstructedPixelsPerSensorsinBox[sensor][0])):
            index_obstructedPixelsPerCurrentSensor = index_obstructedPixelsPerSensorsinBox[sensor][0][ii]
            weightedMap[int(np.shape(labeledMatrixPixel)[0]-1-index_obstructedPixelsPerCurrentSensor[1])][int(index_obstructedPixelsPerCurrentSensor[0])] += weightPerCurrentSensorRegion
...
def computeCoveredAreaofTypicalSensor(selectedSensors, sensorRadius, coordSensors, coordPixels, obstructedPixelsinBox, labeledPixelMatrix, pixelLength, pixelWidth):    
    numAddedPixels = 0
    for pixel in range(len(obstructedPixelsinBox[0])):
        for sensor in range(len(selectedSensors)):
            if pixelisInCircle(selectedSensors[sensor],sensorRadius,int(obstructedPixelsinBox[0][pixel]),coordPixels,coordSensors,pixelLength, pixelWidth):
                numAddedPixels += 1
                break
            
    numTotalCoveredPixels = numAddedPixels
    coverageOfTypicalSensor = numTotalCoveredPixels/len(obstructedPixelsinBox[0])*100
...
def computeWeightedAgeofTypicalSensor_1(selectedSensors, sensorRadius, coordSensors, coordPixels, labeledPixels, labeledMatrixPixel, obstructedPixelsinBox, weightedMap, weightedRegionsPerSensor, lam, d, N, capacity, mu,pixelLength, pixelWidth, k):
    ...
    for pixel in range(len(obstructedPixelsinBox[0])):
        obstructedPixel = obstructedPixelsinBox[0][pixel]
...
def findAnchorPointsPerSensors(N,coordSensors,weightedMap,labeledMatrixPixel,mapOfAnchorPoints,labelofAnchorPoints,pixelLength,pixelWidth,sensorRadius):
    ...
    for sensor in range(N):
        ...
        for anchor_point in range(len(labelofAnchorPoints)):
            ...
            if pixelisInCircle(sensor,sensorRadius,0,coordPixels_new,coordSensors,pixelLength,pixelWidth) == 1:
                #Append the sensors that see the current pixel
                anchorPointsPerSensor[sensor].append(anchor_point)
    
    return anchorPointsPerSensor

def createMapofAnchorPoints(N,weightedMap,labeledMatrixPixel,labeledPixels):
    ...
    for pixel in range(len(labeledPixels)):  
        ...
        if weightPixel > 0:
            ...
            labelofAnchorPoints.append(temp)
            temp += 1
            
    return mapOfAnchorPoints, labelofAnchorPoints
...
def compute_b_age(N, d, mu, partitionsWeight, setofSelectedSensors, setofSensors ,ratePerSensor, currSensor, allPossibleSets,weightedMap,lam):
    b = 0.
    AgePerPartition = []
    coveredWeight = []
    tempP = np.zeros(len(partitionsWeight))
...
def SensSelecModel_1_Greedy_myalgo(N, d, capacity, mu, weightedMap, partitionsWeight , allPossibleSets, setofAlreadySelectedSensors, rectangleLength, rectangleWidth, sensorRadius, k, thresh = 2.):
    #coverageArea = np.sum(partitionsWeight)
    numSelectedSensors = len(N)
    setofSelectedSensors = setofAlreadySelectedSensors
    setofSensors = N
    ...
    for ii in range(int(numSelectedSensors)):
        ...
        for jj in range(len(N)):
            if N[jj] not in setofSelectedSensors:
                b_new = compute_b_age(len(N), d, mu, partitionsWeight, setofSelectedSensors, setofSensors, ratePerSensor, N[jj], allPossibleSets,weightedMap, lam)
                if np.abs(b_new - b_old) >= new_max:
                    new_max = (b_new - b_old)
                    temp_b_old = b_new
                    selectedSensor = N[jj]
...
def SensSelecMyAlgo(N,d,capacity,mu,allPossibleSets,partitionsWeight,rectangleLength,rectangleWidth,coordSensors,weightedMap,sensorRadius,labeledMatrixPixel,labeledPixels,pixelLength,pixelWidth,k):
    selectedSensors = []
    anchorPointsPerSensor = []
    # Step 1: For each anchor point, determine the set of sensors that see it
    mapOfAnchorPoints,labelofAnchorPoints = createMapofAnchorPoints(N,weightedMap,labeledMatrixPixel,labeledPixels)
    anchorPointsPerSensor = findAnchorPointsPerSensors(N,coordSensors,weightedMap,labeledMatrixPixel,mapOfAnchorPoints,labelofAnchorPoints,pixelLength,pixelWidth,sensorRadius) #P_v, for all v
    ...
    S_t = []
    t = 0
    while t <= k-1 and np.sum(np.sort(P_S_t) != labelofAnchorPoints): 
        ...
        for vv in range(len(V1)):
            if len(P_S_t)>0:
                ...
                inter_Pv_PS_t = []
            if len(P[V1[vv]])-len(inter_Pv_PS_t) >= new_max_age_value:
                new_max_age_value = len(P[V1[vv]])-len(inter_Pv_PS_t)
    ...
    setofAlreadySelectedSensors = S_t 
    ...
    if len(S_t) < k:
        ...
        newSelectedSensors = SensSelecModel_1_Greedy_myalgo(remaining_unselected_sensors, d, capacity, mu, weightedMap, partitionsWeight , allPossibleSets, setofAlreadySelectedSensors, rectangleLength, rectangleWidth, sensorRadius, k-len(S_t), thresh = 2.)
    
    if len(newSelectedSensors) > 0:    
        selectedSensors = np.concatenate([S_t,newSelectedSensors])
    else:
        selectedSensors = S_t
...
def computeSubmodularFunc(d, k, weightedAgeTypicalSensor, coverageTypicalSensorSensSelec, num_anchor_points, weightedRegionsPerSensor):
    submodularFunc = 0
    
    #lam = d*(1.+2./3.*k)
    lam = 1e6
    submodularFunc = lam*(coverageTypicalSensorSensSelec*num_anchor_points)/100*weightedRegionsPerSensor - weightedAgeTypicalSensor


    return submodularFunc
...
def main(T=int(1e2)): 
    ...
    scalingFactor = 1
    scale = 1
    N = np.arange(10,11) # number of sensors
    #k = 8
    lam = 1.
...
#            else:
    ...
    stdcoverageTypicalSensorSensSelec_MyAlgo = []
    stdweightedAgeSensSelec_MyAlgo = []
    ...
    for ii in tqdm(range(len(N))):
         ...
         coverageTypicalSensorSensSelec_MyAlgo.append([])
         weightedAgeSensSelec_MyAlgo.append([])
         submodFunc_MyAlgo.append([])
         
         coverageObstructedPixelsSensSelec_1.append([])
         coverageTypicalSensorSensSelec_1.append([])
         weightedAgeSensSelec_1.append([])
         submodFunc_SensSelec_1.append([])
         ...
         stdcoverageTypicalSensorSensSelec_MyAlgo.append([])
         stdweightedAgeSensSelec_MyAlgo.append([])
         ...
         temp2coverageTypicalSensorSensSelec_MyAlgo = []
         temp2WeightedAgeSensSelec_MyAlgo = []
         temp2SubmodFunc_MyAlgo = []
         
         temp2coverageObstructedPixelsSensSelec_1 = []
         temp2coverageTypicalSensorSensSelec_1 = []
         temp2WeightedAgeSensSelec_1 = []
         temp2SubmodFunc_SensSelec_1 = []
         ...
         # Place the sensors on the highway
         print('...start')
         ...
         for jj in range(numIter):
             ...
             temp1coverageTypicalSensorSensSelec_MyAlgo = []
             temp1WeightedAgeSensSelec_MyAlgo = []
             temp1SubmodFunc_MyAlgo = []
             
             temp1coverageObstructedPixelsSensSelec_1 = []
             temp1coverageTypicalSensorSensSelec_1 = []
             temp1WeightedAgeSensSelec_1 = []
             temp1SubmodFunc_SensSelec_1 = []
             ...
             temp2coverageTypicalSensorSensSelec_MyAlgo.append([])            
             temp2WeightedAgeSensSelec_MyAlgo.append([])
             temp2SubmodFunc_MyAlgo.append([])
             ...
             temp2SubmodFunc_SensSelec_1.append([])
             ...
             num_anchor_points = 20
             startTime = time.time()
             # Step 1: Find pixels of interest per sensor
             obstructedPixelsinBox_1, index_obstructedPixelsPerSensorsinBox_1 = findPixelsofInterestPerSensor(labeledMatrixPixel,rectangleLength,rectangleWidth,N[ii],num_anchor_points)
             # Step 2: Weight pixels per sensor
             weightedRegionsPerSensor_1 = placeWeightPerSensor(labeledMatrixPixel,unwantedPixelsCorners1,intersection_sizes,N[ii],num_anchor_points)
             # Step 3: Place a weight on the "pixels of interest"
             weightedMap_1 = weightPixels(obstructedPixelsinBox_1,index_obstructedPixelsPerSensorsinBox_1,weightedRegionsPerSensor_1,labeledMatrixPixel,unwantedPixelsCorners1,intersection_sizes,N[ii])
            ...
            # Step : Compute the different partitions areas
             ...
             startTime = time.time()
             partitionsWeights_1 , allPossibleSets_1 = findPartitionsWeightsFaster(pixelLength, pixelWidth, coordPixels, coordSensors, sensorRadius, labeledPixels, labeledMatrixPixel, N[ii], carDimensions, boxDim, weightedMap_1)
#             partitionsWeights_2 , allPossibleSets_2 = findPartitionsWeights(pixelLength, pixelWidth, coordPixels, coordSensors, sensorRadius, labeledPixels, labeledMatrixPixel, N[ii], carDimensions, boxDim, obstructedLabeledPixelsperSensor_2, regionID_1, weightedMap_1, IDmap_1)
             ...
             for kk in tqdm(range(len(k))):
                 ...
                 tempselectedSensorsMyAlgo = SensSelecMyAlgo(N[ii],d,capacity,mu,allPossibleSets_1,partitionsWeights_1,rectangleLength,rectangleWidth,coordSensors,weightedMap_1,sensorRadius,labeledMatrixPixel,labeledPixels,pixelLength,pixelWidth,k[kk])
    ...
    # =============================================================================
                 ...
                 temp3coverageTypicalSensor_MyAlgo = []
                 temp3weightedAgeTypicalSensor_MyAlgo = []
                 ...
                 for mm in range(num):
                    ...
                    coverageTypicalSensor_Baseline = computeCoveredAreaofTypicalSensor(np.arange(0,N[ii],1), sensorRadius, coordSensors, coordPixels, obstructedPixelsinBox_1[mm], labeledMatrixPixel,pixelLength, pixelWidth)
                    weightedAgeTypicalSensor_Baseline = computeWeightedAgeofTypicalSensor_1(np.arange(0,N[ii],1), sensorRadius, coordSensors, coordPixels, labeledPixels, labeledMatrixPixel, obstructedPixelsinBox_1[mm], weightedMap_1, weightedRegionsPerSensor_1[mm], lam, d, N[ii], capacity, mu, pixelLength, pixelWidth, k=N[ii])
                    
                    # Technique 0: My algo
                    coverageTypicalSensor_MyAlgo = computeCoveredAreaofTypicalSensor(tempselectedSensorsMyAlgo, sensorRadius, coordSensors, coordPixels, obstructedPixelsinBox_1[mm], labeledMatrixPixel,pixelLength, pixelWidth)
                    weightedAgeTypicalSensor_MyAlgo = computeWeightedAgeofTypicalSensor_1(tempselectedSensorsMyAlgo, sensorRadius, coordSensors, coordPixels, labeledPixels, labeledMatrixPixel, obstructedPixelsinBox_1[mm], weightedMap_1, weightedRegionsPerSensor_1[mm], lam, d, N[ii], capacity, mu, pixelLength, pixelWidth, k[kk])
                    
                    # Technique 1: Region of Interest
                    # a - No age minimization
                    coverageTypicalSensor_1 = computeCoveredAreaofTypicalSensor(tempselectedSensorsSensSelec_1, sensorRadius, coordSensors, coordPixels, obstructedPixelsinBox_1[mm], labeledMatrixPixel,pixelLength, pixelWidth)
                    weightedAgeTypicalSensor_1 = computeWeightedAgeofTypicalSensor_1(tempselectedSensorsSensSelec_1, sensorRadius, coordSensors, coordPixels, labeledPixels, labeledMatrixPixel, obstructedPixelsinBox_1[mm], weightedMap_1, weightedRegionsPerSensor_1[mm], lam, d, N[ii], capacity, mu, pixelLength, pixelWidth, k[kk])
                 
                    # b - With age minimization
                    weightedAgeTypicalSensorAgeMin_1 = computeWeightedAgeofTypicalSensor_AgeMin_1(tempselectedSensorsSensSelec_1, sensorRadius, coordSensors, coordPixels, labeledPixels, labeledMatrixPixel, obstructedPixelsinBox_1[mm], weightedMap_1, weightedRegionsPerSensor_1[mm], lam, d, N[ii], capacity, mu, tempRatesofselectedSensorsAgeMin_1, pixelLength, pixelWidth, k[kk])
                 
                    # Technique 2: All box
                    coverageTypicalSensor_2 = computeCoveredAreaofTypicalSensor(tempselectedSensorsSensSelec_2, sensorRadius, coordSensors, coordPixels, obstructedPixelsinBox_1[mm], labeledMatrixPixel,pixelLength, pixelWidth)
                    weightedAgeTypicalSensor_2 = computeWeightedAgeofTypicalSensor_1(tempselectedSensorsSensSelec_2, sensorRadius, coordSensors, coordPixels, labeledPixels, labeledMatrixPixel, obstructedPixelsinBox_1[mm], weightedMap_1, weightedRegionsPerSensor_1[mm], lam, d, N[ii], capacity, mu, pixelLength, pixelWidth, k[kk])
                    ...
                    temp3coverageTypicalSensor_MyAlgo.append(coverageTypicalSensor_MyAlgo)
                    temp3weightedAgeTypicalSensor_MyAlgo.append(weightedAgeTypicalSensor_MyAlgo) 
                 ...
                 coverageTypicalSensor_MyAlgo = np.sum(temp3coverageTypicalSensor_MyAlgo)/num
                 weightedAgeTypicalSensor_MyAlgo  = np.sum(temp3weightedAgeTypicalSensor_MyAlgo)/num
                 ...
                 ## Technique 0: My algo
                 temp1coverageTypicalSensorSensSelec_MyAlgo.append(coverageTypicalSensor_MyAlgo)
                 temp1WeightedAgeSensSelec_MyAlgo.append(weightedAgeTypicalSensor_MyAlgo)
                 ...
                 ## Compute submodular functions
                 ## a- My algo
                 tempsubmodFunc_MyAlgo = computeSubmodularFunc(d,k[kk],weightedAgeTypicalSensor_MyAlgo, coverageTypicalSensor_MyAlgo, num_anchor_points, weightedRegionsPerSensor_1[0])
                 temp1SubmodFunc_MyAlgo.append(tempsubmodFunc_MyAlgo)
                 
                 # b- Sens select 1
                 tempsubmodFunc_SensSelec_1 = computeSubmodularFunc(d,k[kk],weightedAgeTypicalSensor_1, coverageTypicalSensor_1, num_anchor_points, weightedRegionsPerSensor_1[0])
                 temp1SubmodFunc_SensSelec_1.append(tempsubmodFunc_SensSelec_1)                 
             ...
             temp2coverageTypicalSensorSensSelec_MyAlgo[jj].append(temp1coverageTypicalSensorSensSelec_MyAlgo)
             temp2WeightedAgeSensSelec_MyAlgo[jj].append(temp1WeightedAgeSensSelec_MyAlgo)             
             temp2SubmodFunc_MyAlgo[jj].append(temp1SubmodFunc_MyAlgo)
             ...
             temp2SubmodFunc_SensSelec_1[jj].append(temp1SubmodFunc_SensSelec_1)
         ...
         submodFunc_SensSelec_1[ii].append(np.sum(temp2SubmodFunc_SensSelec_1,axis=0)/numIter)
         #weightedAgeSensSelec.append(np.sum(temp1WeightedAgeSensSelec)/numIter*1000.)
         #selectedSensorsSensSelec.append(np.sum(temp1selectedSensorsSensSelec)/numIter)

         coverageTypicalSensorSensSelec_MyAlgo[ii].append(np.sum(temp2coverageTypicalSensorSensSelec_MyAlgo,axis=0)/numIter)
         weightedAgeSensSelec_MyAlgo[ii].append(np.sum(temp2WeightedAgeSensSelec_MyAlgo,axis=0)/numIter*1000.)
         submodFunc_MyAlgo[ii].append(np.sum(temp2SubmodFunc_MyAlgo,axis=0)/numIter)

         stdcoverageTypicalSensorSensSelec_MyAlgo[ii].append(np.std(temp2coverageTypicalSensorSensSelec_MyAlgo,axis=0))
         stdweightedAgeSensSelec_MyAlgo[ii].append(np.std(temp2WeightedAgeSensSelec_MyAlgo,axis=0)*1000.)
...
#    plt.show()
    ...
    sio.savemat('coverageTypicalSensorSensSelec_MyAlgo-k.mat', {'coverageTypicalSensorSensSelec_MyAlgo':coverageTypicalSensorSensSelec_MyAlgo})
    ...
    sio.savemat('stdcoverageTypicalSensorSensSelec_MyAlgo-k.mat', {'stdcoverageTypicalSensorSensSelec_MyAlgo':stdcoverageTypicalSensorSensSelec_MyAlgo})
    ...
    sio.savemat('weightedAgeSensSelec_MyAlgo-k.mat', {'weightedAgeSensSelec_MyAlgo':weightedAgeSensSelec_MyAlgo})
    ...
    sio.savemat('stdweightedAgeSensSelec_MyAlgo-k.mat', {'stdweightedAgeSensSelec_MyAlgo':stdweightedAgeSensSelec_MyAlgo})
    ...
    sio.savemat('submodFunc_MyAlgo-k.mat', {'submodFunc_MyAlgo':submodFunc_MyAlgo})
    sio.savemat('submodFunc_SensSelec_1-k.mat', {'submodFunc_SensSelec_1':submodFunc_SensSelec_1})
