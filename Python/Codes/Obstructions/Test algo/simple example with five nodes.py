...
def findPixelsofInterestPerSensor(labeledMatrixPixel,boxDim,N):
    ...
    for sensor in range(N):
        ...
        obstructedPixelsPerSensors[sensor].append([label_pixel_overlap_left,label_pixel_overlap_right])
        index_obstructedPixelsPerSensors[sensor].append([index_pixel_overlap_left,index_pixel_overlap_right])
...
def weightPixels(obstructedPixelsinBox,index_obstructedPixelsPerSensorsinBox,weightedRegionsPerSensor,labeledMatrixPixel,boxDim,N):
    ...
    for sensor in range(1):
        ...
        index_pixel_overlap_top = index_obstructedPixelsPerCurrentSensor[0][0]
        index_pixel_overlap_bottom = index_obstructedPixelsPerCurrentSensor[0][1]
        
        weightPerCurrentSensorRegion = weightedRegionsPerSensor[sensor]
        # Weight the corresponding pixels
        weightedMap[int(np.shape(labeledMatrixPixel)[0]-1-index_pixel_overlap_top[1])][int(index_pixel_overlap_top[0])] += 2
        weightedMap[int(np.shape(labeledMatrixPixel)[0]-1-index_pixel_overlap_bottom[1])][int(index_pixel_overlap_bottom[0])] += 1
...
def main(T=int(1e2)): 
    ...
    scalingFactor = 1
    scale = 1
    N = np.arange(5,6) # number of sensors
    #k = 8
    lam = 1.
