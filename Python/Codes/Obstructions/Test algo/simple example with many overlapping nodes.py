...
def findPixelsofInterestPerSensor(labeledMatrixPixel,boxDim,N):
    ...
    index_pixel_overlap_left = np.array([50 , boxDim[1]/2])
    ...
    index_pixel_overlap_right = np.array([boxDim[0]-50, boxDim[1]/2])
...
def weightPixels(obstructedPixelsinBox,index_obstructedPixelsPerSensorsinBox,labeledMatrixPixel,boxDim,N):
    ...
    for sensor in range(1):
        ...
        weightedMap[int(np.shape(labeledMatrixPixel)[0]-1-index_pixel_overlap_left[1])][int(index_pixel_overlap_left[0])] += 2
        weightedMap[int(np.shape(labeledMatrixPixel)[0]-1-index_pixel_overlap_center[1])][int(index_pixel_overlap_center[0])] += 1
...
def main(T=int(1e2)): 
    ...
    for ii in tqdm(range(len(N))):
         ...
         for jj in range(numIter):
             ...
             coordSensors.append(np.array([10, boxDim[1]/2]))
             
             
             coordSensors.append(np.array([boxDim[0]/2 - 15, boxDim[1]/2]))   
             coordSensors.append(np.array([boxDim[0]/2 + 15, boxDim[1]/2]))   
             
             coordSensors.append(np.array([boxDim[0] - 10, boxDim[1]/2]))   
