...
def findPixelsofInterestPerSensor(labeledMatrixPixel,unwantedPixelsCorners1,intersection_sizes,N):
    obstructedPixelsPerSensors = []
    index_obstructedPixelsPerSensors = []
    
    # Pixels of interest in the different regions:
    # Road 1: Right side
    index_pixel_road1_right = np.array([unwantedPixelsCorners1[1][0]+intersection_sizes[0]+unwantedPixelsCorners1[1][0]/2,unwantedPixelsCorners1[2][1]+intersection_sizes[1]/2])
    ...
    # Road 2: Up side
    index_pixel_road2_up = np.array([unwantedPixelsCorners1[1][0]+intersection_sizes[0]/2,unwantedPixelsCorners1[2][1]+intersection_sizes[1]+unwantedPixelsCorners1[2][1]/2])
    ...
    for sensor in range(N):
        ...
        obstructedPixelsPerSensors[sensor].append([label_pixel_road1_right,label_pixel_road2_up])
        index_obstructedPixelsPerSensors[sensor].append([index_pixel_road1_right,index_pixel_road2_up])
        
        
    return obstructedPixelsPerSensors,index_obstructedPixelsPerSensors


def placeWeightPerSensor(labeledMatrixPixel,unwantedPixelsCorners1,intersection_sizes,N):
...
def weightPixels(obstructedPixelsinBox,index_obstructedPixelsPerSensorsinBox,weightedRegionsPerSensor,labeledMatrixPixel,unwantedPixelsCorners1,intersection_sizes,N):
    ...
    for sensor in range(N):
        ...
        index_pixel_road1_right = index_obstructedPixelsPerCurrentSensor[0][0]
        index_pixel_road2_up = index_obstructedPixelsPerCurrentSensor[0][1]
        
        weightPerCurrentSensorRegion = weightedRegionsPerSensor[sensor]
        # Weight the corresponding pixels
        weightedMap[int(np.shape(labeledMatrixPixel)[0]-1-index_pixel_road1_right[1])][int(index_pixel_road1_right[0])] += weightPerCurrentSensorRegion + 1e-3/24
        weightedMap[int(np.shape(labeledMatrixPixel)[0]-1-index_pixel_road2_up[1])][int(index_pixel_road2_up[0])] += weightPerCurrentSensorRegion
...
def compute_b_1(N, d, mu, partitionsWeight, setofSelectedSensors, setofSensors ,ratePerSensor, gamma_right, gamma_up, currSensor, allPossibleSets,weightedMap,lam):
    b = 0.
    AgePerPartition = []
    coveredWeight = []
    ...
    for ii in range(len(newPartitionWeight)):
        n = tempP[ii]
        if n!=0 and newPartitionWeight[ii]>12:
            tempAge = 1-(1-ratePerSensor*(gamma_right-d))**n
        elif n!=0:
            tempAge = 1-(1-ratePerSensor*(gamma_up-d))**n
            #tempAge = (n+2.)/(n+1.)*(1./ratePerSensor)
            if np.isnan(tempAge):
...
def compute_b_2(N, d, mu, partitionsArea, setofSelectedSensors, gamma_right, gamma_up, setofSensors ,ratePerSensor, currSensor, allPossibleSets, lam):
    b = 0.
    AgePerPartition = []
    coveredArea = []
...
def SensSelecModel_1_StochGreedy(N, d, capacity, mu, weightedMap, partitionsWeight , allPossibleSets, rectangleLength, rectangleWidth, sensorRadius, scalingFactor, lam, k, thresh = 2.):
    ...
    gamma_right = d + 0.5/ratePerSensor
    gamma_up = d + 0.5/ratePerSensor
    ...
    for ii in range(int(numSelectedSensors)):
        ...
        for jj in range(len(setOfRandomGeneratedSensors)):
            if setOfRandomGeneratedSensors[jj] not in setofSelectedSensors:
                b_new, temp_percentageCoveredWeight , temp_weightedAge , selectedPartitionsArea = compute_b_1(N, d, mu, partitionsWeight, setofSelectedSensors, setofSensors, ratePerSensor, gamma_right, gamma_up, setOfRandomGeneratedSensors[jj], allPossibleSets,weightedMap, lam)
...
def SensSelecModel_2_StochGreedy(N, d, capacity, mu, partitionsArea , allPossibleSets, rectangleLength, rectangleWidth, sensorRadius, scalingFactor, lam, k, thresh = 2.):
    ...
    for ii in range(int(numSelectedSensors)):
        ...
        for jj in range(len(setOfRandomGeneratedSensors)):
            if setOfRandomGeneratedSensors[jj] not in setofSelectedSensors:
                b_new, tempcoverageArea, tempareaWeightedAge, selectedPartitionsArea = compute_b_2(N, d, mu, partitionsArea, setofSelectedSensors, gamma_right, gamma_up, setofSensors, ratePerSensor, setOfRandomGeneratedSensors[jj], allPossibleSets, lam)
                if np.abs(b_new - b_old) >= new_max:
                    new_max = (b_new - b_old)
                    temp_b_old = b_new
                    selectedSensor = setOfRandomGeneratedSensors[jj]
...
def main(T=int(1e2)): 
    ...
    scalingFactor = 1
    scale = 1
    N = np.arange(24,25) # number of sensors
    #k = 8
    lam = 1.
    ...
    for ii in tqdm(range(len(N))):
         ...
         for jj in range(numIter):
             ...
             coordSensors.append(np.array([unwantedPixelsCorners1[1][0]+2,unwantedPixelsCorners1[2][1]+intersection_sizes[1]/pixelWidth+20]))                                      
             sensors_road.append([2])
             coordSensors.append(np.array([unwantedPixelsCorners1[1][0]+intersection_sizes[0]-2,unwantedPixelsCorners1[2][1]+intersection_sizes[1]/pixelWidth+20]))
             sensors_road.append([2])        
             
             coordSensors.append(np.array([unwantedPixelsCorners1[1][0]+2,unwantedPixelsCorners1[2][1]+intersection_sizes[1]/pixelWidth+30]))                                      
             sensors_road.append([2])
             coordSensors.append(np.array([unwantedPixelsCorners1[1][0]+intersection_sizes[0]-2,unwantedPixelsCorners1[2][1]+intersection_sizes[1]/pixelWidth+30]))
             sensors_road.append([2])      
             
             # Road 1 producers: right side
             coordSensors.append(np.array([unwantedPixelsCorners1[1][0]+intersection_sizes[0]/pixelLength+30,unwantedPixelsCorners1[3][1]+3]))                                      
             sensors_road.append([2])
             coordSensors.append(np.array([unwantedPixelsCorners1[1][0]+intersection_sizes[0]/pixelLength+30,unwantedPixelsCorners1[3][1]+3]))
             sensors_road.append([2])       
             
             coordSensors.append(np.array([unwantedPixelsCorners1[1][0]+intersection_sizes[0]/pixelLength+40,unwantedPixelsCorners1[3][1]+3]))                                      
             sensors_road.append([2])
             coordSensors.append(np.array([unwantedPixelsCorners1[1][0]+intersection_sizes[0]/pixelLength+40,unwantedPixelsCorners1[3][1]+3]))
             sensors_road.append([2])               
             #################################################################################################
             ...
             coordSensors.append(np.array([boxDim[0]-30,unwantedPixelsCorners1[3][1]+3]))                                      
             sensors_road.append([1])
             coordSensors.append(np.array([boxDim[0]-30,unwantedPixelsCorners1[3][1]+intersection_sizes[1]/pixelWidth-3]))
             sensors_road.append([1])
             
             coordSensors.append(np.array([boxDim[0]-40,unwantedPixelsCorners1[3][1]+3]))                   
             sensors_road.append([1])
             coordSensors.append(np.array([boxDim[0]-40,unwantedPixelsCorners1[3][1]+intersection_sizes[1]/pixelWidth-3]))
             sensors_road.append([1])
...
#             coordSensors = np.concatenate((xcoordSensors,ycoordSensors),axis=1)             
             ...
             obstructedPixelsinBox_1, index_obstructedPixelsPerSensorsinBox_1 = findPixelsofInterestPerSensor(labeledMatrixPixel,unwantedPixelsCorners1,intersection_sizes,N[ii])
             # Step 2: Weight pixels per sensor
             weightedRegionsPerSensor_1 = placeWeightPerSensor(labeledMatrixPixel,unwantedPixelsCorners1,intersection_sizes,N[ii])
