...
# =============================================================================
#     max_length_set_newallPossibleSets = len(newallPossibleSets[-1])
#     counter = 0
#     for s in range(len(allPossibleSets)):
#         temp = []
#         if allPossibleSets[s] in newallPossibleSets:
#             #idx = np.where(allPossibleSets[s] == newallPossibleSets[counter])[0][0]
#             newselectedPartitionsArea[counter] = newselectedPartitionsArea[counter] + partitionsArea[s]
#             if len(allPossibleSets[s]) < max_length_set_newallPossibleSets:
#                 counter = counter + 1           
#         elif len(allPossibleSets[s]) > max_length_set_newallPossibleSets: 
#             temp1 = []
#             for ii in range(len(allPossibleSets[s])):
#                 temp1.append(allPossibleSets[s][ii])            
#             hello = findsubsets(np.array(temp1),max_length_set_newallPossibleSets)
#             for jj in range(len(hello)):
#                 temp.append(list(hello[jj]))
#             for jj in range(len(temp)):
#                 if temp[jj] in newallPossibleSets:
#                     #idx = np.where(newallPossibleSets == allPossibleSets[s])[0][0]
#                     newselectedPartitionsArea[counter] = newselectedPartitionsArea[counter] + partitionsArea[s]
#             counter = counter + 1  
# =============================================================================
...
def main(T=int(5e2)): 
    scalingFactor = 50
    N = np.arange(2,16,1) # number of sensors
    lam = 2.
